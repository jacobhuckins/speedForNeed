...
void matrixQR(float* a, int rows, int cols, float* q, float* r) {
    float* extracted_a = (float*)malloc(rows * sizeof(float));
    float* extracted_q = (float*)malloc(rows * sizeof(float));
    
    for (int j = 0; j < cols; j++) {
        matrixExtractCol(a, rows, cols, j, extracted_a);
        
        for (int i = 0; i < j; i++) {
            matrixExtractCol(q, rows, cols, i, extracted_q);
            r[i * cols + j] = vectorDotProduct(extracted_q, extracted_a, rows, rows);
            
            for (int k = 0; k < rows; k++) {
                extracted_a[k] -= r[i * cols + j] * extracted_q[k];
            }
        }
        
        r[j * cols + j] = vectorMag(extracted_a, rows);
        
        if (r[j * cols + j] != 0) {
            float* normalized = vectorNorm(extracted_a, rows);
            for (int k = 0; k < rows; k++) {
                q[k * cols + j] = normalized[k];
            }
        }
    }
}
